...
- [General Design](#general-design)
	...
	- [3. Request Query Options](#3-request-query-options)
		...
		- [3.16. Phrase Query Options](#316-phrase-query-options)
			- [Terms](#terms)
			- [Field](#field)
		- [3.17. Fuzzy Query Options](#317-fuzzy-query-options)
			- [Term](#term)
			- [Prefix Length](#prefix-length)
			- [Fuzziness](#fuzziness)
			- [Field](#field)
		- [3.18. Prefix Query Options](#318-prefix-query-options)
			- [Prefix](#prefix)
			- [Field](#field)
...
Some queries are more limited and usually used for debugging purposes:
 ...
 - **TermQuery:** A query that looks for **exact** matches of the term in the index (no analyzer, no stemming). Useful to check what the actual content of the index is. Usual better alternative is `MatchQuery`.
 - **Fuzzy Query:** A fuzzy query is a term query that matches terms within a specified edit distance (Levenshtein distance). Usual better alternative is `MatchQuery`.
...
```java
new TermQuery("someterm").field("fieldname")
```
