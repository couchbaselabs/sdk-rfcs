...
```java
bucket.query(String indexName, SearchQuery query[, SearchParams params]);
```
...
`SearchQuery` are composites (you can nest queries inside some specific query types), and the search parameters (like limit, explain) are only expected at top level of the payload, hence the separation of `SearchQuery` and the optional `SearchParams`.

Here is an example of a String query with some search params, on index `beer-index`:

```java
bucket.query("beer-index", new StringQuery("description:awesome").boost(2), new SearchParams().limit(5))
```
...
```java
bucket.query("beer-index", SearchQuery.string("+water +abv:>10"), new SearchParams().limit(5))
```
...
`SearchQuery` will be represented as a JSON Object under the `query` top-level attribute, while each individual tuning of the `SearchParams` will have its own top-level attribute in the JSON payload:

```json
{
    "query": { "represented by": "SearchQuery" },
    "limit": "this is a value from the SearchParams",
    "explain": "this is another value from the SearchParams"
}
```

In order to generate the JSON payload above, an `export()` method can be added to the `SearchQuery` interface. In this case, it should accept a `SearchParams` as an optional parameter and generate the whole JSON payload, including parameters.
...
```java
new ConjunctionQuery(SearchQuery queries....).and(SearchQuery... someMoreQueries)
```

| Builder Name | Request Path | Wire Type | Default | Constraints |
| ------------ | ------------ | --------- | ------- | ----------- |
| `(constructor argument) SearchQuery... queries` | `query.conjuncts` | JSON Array | - | - |
| `and(SearchQuery... moreQueries)` | `query.conjuncts` | JSON Array | - | - |
...
```java
new DisjunctionQuery(SearchQuery queries....).or(SearchQuery... moreQueries)
```

| Builder Name | Request Path | Wire Type | Default | Constraints |
| ------------ | ------------ | --------- | ------- | ----------- |
| `(constructor argument) SearchQuery... queries` | `query.disjuncts` | JSON Array | - | - |
| `or(SearchQuery... moreQueries` | `query.disjuncts` | JSON Array | - | - |
...
| Builder Name | Request Path | Wire Type | Default | Constraints |
| ------------ | ------------ | --------- | ------- | ----------- |
| `must(SearchQuery... query)` | `query.must` | a `ConjunctionQuery` object of the must queries | - | - |
| `should(SearchQuery... query)` | `query.should` | a `DisjunctionQuery` object of the should queries | - | - |
| `mustNot(SearchQuery... query)` | `query.must_not` | a `DisjunctionQuery` object of the must_not queries | - | - |
