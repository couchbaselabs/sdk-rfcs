...
The Bucket API thus only takes a `SearchQuery`:

```java
bucket.query(new SearchQuery(/*indexName, queryPart[, searchParams]*/);
```
...
`AbstractFtsQuery` are composites (you can nest queries inside some specific query types), and the search parameters (like limit, explain) are only expected at top level of the payload, hence the separation of `AbstractFtsQuery ` and the optional `SearchParams`.
...
SearchQuery sq = new SearchQuery("beer-index",
    new StringQuery("description:awesome").boost(2),
    new SearchParams().limit(5)
);
bucket.query(sq);
```

If idiomatic by the language, it makes sense to provide static factory methods (or similar helpers) to guide the user in the right direction since there are many different query options available. These should be exposed on the bucket-level wrapper class. So for example the `SearchQuery` can contain factory methods in a form like this:

```java
bucket.query(new SearchQuery("beer-index", SearchQuery.string("+water +abv:>10"), new SearchParams().limit(5)));
```
...
`AbstractFtsQuery` will be represented as a JSON Object under the `query` top-level attribute, while each individual tuning of the `SearchParams` will have its own top-level attribute in the JSON payload:

```json
{
    "query": { "represented by": "AbstractFtsQuery" },
    "limit": "this is a value from the SearchParams",
    "explain": "this is another value from the SearchParams"
}
```

In order to generate the JSON payload above, an `export()` method can be added to the `AbstractFtsQuery ` interface. In this case, it should accept a `SearchParams` as an optional parameter and generate the whole JSON payload, including parameters.

The `indexName` part of the request is not part of the payload but rather determines which HTTP endpoint to hit.
...
| Builder Name | Request Path | Wire Type | Default | Constraints |
| ------------ | ------------ | --------- | ------- | ----------- |
| `addFacets(SearchFacet...)`    | `facets`     | JSON Object of Objects `{name: {facet}, name: {facet}...}` | no facets | For a single facet, either `numeric_ranges` or `date_ranges` is allowed. If omitted it is a term facet. |

One can add several facets in a single call with this method. Additionally, calling `addFacets` several times is additive: new facets are added to an underlying collection of facets. If a facet is added with the same name as an existing one, it overwrites it (so the recommended backing collection for facets would be a `Map` or other similar structure).
...
| ---------- | -------------- | ----------------- |
| term       | `SearchFacet.term(...)` | - |
| numeric    | `SearchFacet.numeric(...)` | `[ { "name": name of range (string), "min": inclusive lower bound (float), "max": inclusive upper bound (float) } ]` |
| date       | `SearchFacet.date(...)` | `[ { "name": name of range (string), "start": "2011-08-30T13:22:53.108Z", "end": "... quoted string rfc 3339" } ]`  |
...
```java
.addFacets(SearchFacet.term("type", "category", 5))

.addFacets(SearchFacet.numeric("strength", "abv", 3)
    .addRange("strong", 5, null)
    .addRange("middle", 3.1, 4.99)
    .addRange("light", 0, 3.0));

.addFacets(SearchFacet.date("age", "updated", 2)
...
#### Query Consistency
**TODO**
Note that REQUEST_PLUS is slated for spock ([MB-18428]( https://issues.couchbase.com/browse/MB-18428)).This should look like the stuff we have in N1QL, abstracting over the mutation tokens with `consistentWith`.
...
The inner representation of child queries in the `conjuncts` section of the payload should omit any `SearchParams` set on the inner query, but take the `boost` into account.
...
The inner representation of child queries in the `disjuncts` section of the payload should omit any `SearchParams` set on the inner query, but take the `boost` into account.
...
The inner representation of child queries in the `must/must_not/should` sections are respectively a `ConjunctionQuery` and two `DisjunctionQuery`. As for these types, the inner representation of the payload should omit any `SearchParams` set on the inner query, but take the `boost` into account.
...
| Builder Name | Request Path | Wire Type | Default | Constraints |
| ------------ | ------------ | --------- | ------- | ----------- |
| `docIds(String... docIds)` | `query.ids` | JSON String | - | - |
...
- **I wonder if its possible that the compound queries can be put into one API?**
    ...
    > **simon:** `BooleanQuery` is kind of a good middle ground
- **the response also returns the request, seems a little wasteful on network bandwith?**

    > **simon:** this is actually the request that FTS ended up actually executing. Due to optimizations and parsing, it could be different from the one passed in by the user. But that shouldn't happen with SDKs as they guarantee well-formed requests. Maybe include somewhere as a TRACE log?
- **Should `MatchAllQuery` and `MatchNoneQuery` be included at all?**
    > **simon:** **YES**, marty indicated that you cannot do specific use cases without them, so they're bound to become useful to someone at some point.
