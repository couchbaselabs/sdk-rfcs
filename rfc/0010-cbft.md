...
- **I wonder if its possible that the compound queries can be put into one API?**
    >**sba:** `BooleanQuery` is kind of a good middle ground
- **the response also returns the request, seems a little wasteful on network bandwith?**
	>**sba:** this is actually the request that FTS ended up actually executing. Due to optimizations and parsing, it could be different from the one passed in by the user. But that shouldn't happen with SDKs as they guarantee well-formed requests. Maybe include somewhere as a TRACE log?
- **Should `PrefixQuery` be included or can it be merged with another type of query (like FuzzyQuery was merged into `TermQuery`)?**
    > &nbsp;
