...
The first range creation can be represented directly in the facet's constructor (adding the necessary input for a single range to the constructor's params).

The subsequent ranges would then be added through a `addRange` method, in a fluent manner: the `addRange` would return the facet instance, allowing method chaining.
...
addFacets(SearchFacet.numeric("strength", "abv", 3, "strong", 5, null)
    .addRange("middle", 3.1, 4.99)
    .addRange("light", 0, 3.0));

addFacets(SearchFacet.date("age", "updated", 2, "old", null, "2016-01-01T00:00:00")
